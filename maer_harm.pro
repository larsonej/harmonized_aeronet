...
ind=where(finite(maod510[*,i]) eq 1)
if n_elements(ind) gt 5 then begin
ndat2[i,0]=n_elements(ind)
;interpolating in 2D using fancy algorithm,
grid_input, lon[ind],lat[ind],reform(maod510[ind,i]),xyz,newaod,/degrees,/sphere
...
ind2=where(finite(maod1020[*,i]) eq 1)
if n_elements(ind2) gt 5 then begin
ndat2[i,1]=n_elements(ind2)
;interpolating in 2D using fancy algorithm,
grid_input, lon[ind2],lat[ind2],reform(maod1020[ind2,i]),xyz,newaod,/degrees,/sphere
lons=!radeg*atan(xyz[1,*],xyz[0,*])
lats=!radeg*asin(xyz[2,*])
qhull, lons,lats,tri,/delaunay, sphere=sp
...
ind3=where(finite(mae[*,i]) eq 1)
if n_elements(ind3) gt 5 then begin
ndat2[i,2]=n_elements(ind3)
;interpolating in 2D using fancy algorithm,
grid_input, lon[ind3],lat[ind3],reform(mae[ind3,i]),xyz,newaod,/degrees,/sphere
lons=!radeg*atan(xyz[1,*],xyz[0,*])
lats=!radeg*asin(xyz[2,*])
qhull, lons,lats,tri,/delaunay, sphere=sp
altgrid2=griddata(lons,lats,newaod,/sphere,/degrees,/grid,xout=glon,yout=glat,method='NaturalNeighbor',triangles=tri)
...
for k=0,2 do begin
  smaeronet[i,j,*,k]=smooth(aeronet[i,j,*,k],3,/nan)
endfor
endfor
endfor
...
for i=0,1 do begin
ind=where(finite(mnorm[*,i]) eq 1)
grid_input, lon[ind],lat[ind],reform(mnorm[ind,i]),xyz,newaod,/degrees,/sphere
lons=!radeg*atan(xyz[1,*],xyz[0,*])
lats=!radeg*asin(xyz[2,*])
qhull, lons,lats,tri,/delaunay, sphere=sp
...
id=ncdf_create('harmonized_monthly_aeronet.nc',/clobber)
...
;DEFINE ATTRIBUTES
ncdf_attput, id, did, 'units', 'months since 1993-01-01'
...
plot, yrs+mons/12.,gm,thick=2, linestyle=1
oplot, yrs+mons/12.,gm1,linestyle=2,thick=2
oplot, yrs+mons/12.,gm2, thick=2
